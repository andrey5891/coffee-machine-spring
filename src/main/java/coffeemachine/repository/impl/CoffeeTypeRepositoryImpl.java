...
public class CoffeeTypeRepositoryImpl implements coffeemachine.repository.CoffeeTypeRepository {
    ...
    private void fillDbSimulationByBeginValues() {
        this.coffeeTypeMap = new HashMap<>(
          Map.of(
                  1L,CoffeeType.builder().id(1L).name("ESPRESSO").waterAmount(250L).milkAmount(0L)
                          .coffeeBeanAmount(16L).price(4L).build(),
                  2L, CoffeeType.builder().id(2L).name("LATTE").waterAmount(350L).milkAmount(75L)
